...
const Navbar = () => {
  ...
  const Sidebar = () => {
        ...
        <>
          <Link href={route} >
              <div className='border-b-2 border-white text-base font-normal flex items-center p-4'>
                <span className='material-symbols-outlined w-10 text-orange-500'>{symbol}</span><span>{text}</span>
              </div>
            </Link>
        </>
      )
    ...
    return (
      <>
        <div className={"h-screen bg-black fixed w-72 z-10 top-11 md:top-12 md:right-0"}>
  ...
  return (
    <div className='w-full bg-black h-12 max-h-max md:h-14 flex justify-between items-center text-white sticky z-50 top-0 overflow-hidden'>
      ...
      {showSidebar &&<Sidebar />}
      <div className="mx-5 md:mx-9 font-semibold text-xl">BlogWriter</div>
      <ul className='md:flex gap-2 mr-5 hidden'>
        {user && <>
          ...
          <Link href={"/MyBlogs"}><li className='text-sm  hover:bg-[#bababa36] font-normal rounded-3xl py-2 px-5 transition-all'>MY BLOGS</li></Link>
